(function () {
  ...
  function init(rootEl) {
    const prospectPk = rootEl.dataset.prospectPk;
    if (!prospectPk) return;

    const endpoints = {
      list: `/prospects/detail/${prospectPk}/documents/v2/list/`,
      upload: `/prospects/detail/${prospectPk}/documents/upload/`,
      bulkDelete: `/prospects/detail/${prospectPk}/documents/delete/`,
      addNote: (docId) => `/prospects/detail/${prospectPk}/documents/${docId}/notes/add/`,
      deleteNote: (docId, noteId) => `/prospects/detail/${prospectPk}/documents/${docId}/notes/${noteId}/delete/`,
    };
    ...
    function bindHandlers() {
      ...
      function selectedDocIds() {
        return Array.from(rootEl.querySelectorAll(".df2-select-doc:checked")).map((el) => el.dataset.id);
      }
      ...
      openUploaderBtn?.addEventListener("click", () => modalInstance(uploaderModalEl)?.show());
      uploaderZone?.addEventListener("dragover", (e) => {
        e.preventDefault();
        uploaderZone.classList.add("border-primary");
      });
      uploaderZone?.addEventListener("dragleave", () => uploaderZone.classList.remove("border-primary"));
      ...
      rootEl.querySelector("#df2-modal-drop-instructions")?.addEventListener("click", (e) => {
      ...
      rootEl._df2OnRootChange = onRootChange;
      ...
      const onRootClick = async (e) => {
        ...
        if (e.target.closest(".df2-view-notes")) {
          ...
          if (viewNotesBodyEl) viewNotesBodyEl.innerHTML = tpl ? tpl.innerHTML : '<div class="text-muted small">No notes found.</div>';
      ...
      rootEl._df2OnRootClick = onRootClick;
    ...
    loadList().catch((err) => {
      rootEl.innerHTML = `<div class="text-danger">Failed to load documents: ${err.message}</div>`;
    });
  }

  document.addEventListener("DOMContentLoaded", function () {
    const rootEl = document.getElementById("documentsPageRootV2");
